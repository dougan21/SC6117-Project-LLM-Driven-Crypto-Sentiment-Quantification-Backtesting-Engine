...
def build_records_from_backtest(
    backtest_parquet: Path,
    price_col: str = "close",
    initial_capital: float = 100_000.0,
    time_format: str = "iso",
    ...
    """
    Build frontend-friendly records from a backtest parquet without writing to disk.

    :param backtest_parquet: Path to parquet produced by sentiment_strategy_backtest
    :param price_col: price column for buy-and-hold benchmark
    :param initial_capital: scaling for equity
    :param time_format: 'iso' or strftime pattern
    :param points: optional downsample to at most this many points (evenly)
    ...
    if points is not None and points > 0 and len(idxs) > points:
        # even downsample
        step = max(1, len(idxs) / points)
        idxs = [int(i * step) for i in range(points)]
    ...
    for i in idxs:
        ts = df.index[i]
        hold_v = float(hold_equity.iloc[i])
        strat_v = float(strat_equity.iloc[i])

        item = {
            "time": format_time(ts, time_format),
            "holdValue": float(round(hold_v)),
            "strategyValue": float(round(strat_v)),
        }
        evts = events[i]
        if evts:
            item["events"] = evts
