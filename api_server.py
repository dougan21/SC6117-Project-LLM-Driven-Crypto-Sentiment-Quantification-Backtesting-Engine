...
try:
    from lib.klineAcquision import get_price_data_last_n_days, get_price_data_range, parse_hourly_datetime
except Exception:
...
try:
    from lib.sentiment_strategy_backtest import run_backtest as run_backtest_func
except Exception:
    run_backtest_func = None

try:
    from lib.export_backtest_json import export_backtest_json as export_backtest_json_func
except Exception:
    export_backtest_json_func = None
try:
    from lib.export_backtest_json import build_records_from_backtest as build_records_func
except Exception:
...
def api_fetch_klines(req: FetchKlinesRequest, background_tasks: BackgroundTasks):
    if get_price_data_last_n_days is None and get_price_data_range is None:
        raise HTTPException(status_code=500, detail="Kline acquisition functions not available")

    def _job():
        if req.days is not None and get_price_data_last_n_days is not None:
            df = get_price_data_last_n_days(symbol=req.symbol, timeframe=req.timeframe, days=req.days)
        else:
            if not req.start or not req.end:
                raise ValueError("Either 'days' or both 'start' and 'end' must be provided")
            if parse_hourly_datetime is None or get_price_data_range is None:
                raise ValueError("Range fetch not available in this environment")
            start_ts = parse_hourly_datetime(req.start)
            end_ts = parse_hourly_datetime(req.end)
            df = get_price_data_range(symbol=req.symbol, timeframe=req.timeframe, start_time=start_ts, end_time=end_ts)
...
def api_prepare_dataset(req: PrepareDatasetRequest, background_tasks: BackgroundTasks):
    if prepare_dataset_func is None:
        raise HTTPException(status_code=500, detail="prepare_dataset function not available")

    def _job():
        start_ts = parse_hourly_datetime(req.start) if req.start and parse_hourly_datetime else None
        end_ts = parse_hourly_datetime(req.end) if req.end and parse_hourly_datetime else None
        prepare_dataset_func(price_parquet=req.price_parquet, sentiment_csv=req.sentiment_csv, out_path=req.out, decay_span=req.decay_span, start_time=start_ts, end_time=end_ts)
...
def api_run_backtest(req: RunBacktestRequest, background_tasks: BackgroundTasks):
    if run_backtest_func is None:
        raise HTTPException(status_code=500, detail="run_backtest not available")
...
def api_export_backtest(req: ExportBacktestRequest):
    if export_backtest_json_func is None:
        raise HTTPException(status_code=500, detail="export_backtest_json not available")
    try:
        export_backtest_json_func(backtest_parquet=Path(req.backtest_parquet), price_col=req.price_col, output_json=Path(req.output_json), initial_capital=req.initial_capital, time_format=req.time_format)
...
def chart_data(
    backtest_parquet: Optional[str] = Query(None, description="optional backtest parquet path to generate data from"),
    cryptoPair: Optional[str] = Query(None, description="crypto pair, e.g. BTC/USD"),
    price_col: str = Query("close"),
    initial_capital: float = Query(100000.0),
    points: Optional[int] = Query(None, description="downsample to at most this many points"),
    startDateTime: Optional[str] = Query(None),
    endDateTime: Optional[str] = Query(None),
    # legacy aliases accepted by some clients
    start: Optional[str] = Query(None, description="legacy alias for startDateTime"),
    end: Optional[str] = Query(None, description="legacy alias for endDateTime"),
    time_format: str = Query("iso"),
    background_tasks: BackgroundTasks = None,
):
    LOG(f"[Debug] /api/chart-data called with backtest_parquet={backtest_parquet}, startDateTime={startDateTime}, endDateTime={endDateTime}, points={points}")
    ...
    if not backtest_parquet:
        try:
            ...
            if startDateTime or endDateTime:
                try:
                    s_req = datetime.fromisoformat(startDateTime.replace("Z", "+00:00")) if startDateTime else None
                except Exception:
                    s_req = None
                try:
                    e_req = datetime.fromisoformat(endDateTime.replace("Z", "+00:00")) if endDateTime else None
                except Exception:
                    e_req = None
                ...
                for p in sorted(candidates, key=lambda p: p.stat().st_mtime, reverse=True):
                    try:
                        ...
                        idx = df.index if isinstance(df.index, _pd.DatetimeIndex) else None
                        if idx is None:
                            continue
            ...
            if not chosen and startDateTime:
                try:
                    s = datetime.fromisoformat(startDateTime.replace("Z", "+00:00"))
                    month_pattern = f"backtest_{s.year}-{s.month:02d}*.parquet"
                    month_cands = list(Path("results").glob(month_pattern))
                    if month_cands:
                        chosen = sorted(month_cands, key=lambda p: p.stat().st_mtime, reverse=True)[0]
                except Exception:
                    chosen = None
    ...
    if not backtest_parquet:
        try:
            ...
            if endDateTime:
                e_req = datetime.fromisoformat(endDateTime.replace("Z", "+00:00")).astimezone(timezone.utc)

            merged_dir = Path("data/merged")
            merged_files = sorted(merged_dir.glob("*.parquet"), key=lambda p: p.stat().st_mtime, reverse=True)
            used_files = []
            df_slice = None
            if merged_files:
                import pandas as _pd
                for p in merged_files:
                    try:
                        ...
                        if s_req is None and e_req is None:
                            used_files.append(p)
                            df_slice = tmp if df_slice is None else _pd.concat([df_slice, tmp])
                            continue
                        # check overlap
            ...
            if df_slice is not None and len(df_slice):
                ...
                if len(df_req) > 0:
                    ...
                    chosen_parquet = ",".join([str(x) for x in used_files]) if used_files else str(prepared_path)
                    ...
                    if len(df_req) <= SYNC_ROW_LIMIT:
                        ...
                        backtest_runtime = Path("results/backtest_runtime.parquet")
                        run_backtest_func(input_parquet=prepared_path, price_col=price_col, sentiment_col="sentiment_score", output_path=backtest_runtime, params=None)
                        records = build_records_func(backtest_runtime, price_col=price_col, initial_capital=initial_capital, time_format=time_format, points=points)
                        source = "merged-sync"
                        ...
                        backtest_runtime = Path(f"results/backtest_runtime_{job_id}.parquet")
                        ui_json = Path(f"results/backtest_for_ui_runtime_{job_id}.json")

                        def _bg_task(prep: str, out_bt: str, out_json: str):
                            try:
                                run_backtest_func(input_parquet=Path(prep), price_col=price_col, sentiment_col="sentiment_score", output_path=Path(out_bt), params=None)
                                recs = build_records_func(Path(out_bt), price_col=price_col, initial_capital=initial_capital, time_format=time_format, points=points)
                                # write ui json
                        ...
                        if background_tasks is not None:
                            background_tasks.add_task(_bg_task, str(prepared_path), str(backtest_runtime), str(ui_json))
    ...
    if backtest_parquet:
        if build_records_func is None:
            raise HTTPException(status_code=500, detail="server cannot build records from parquet (function missing)")
        try:
            records = build_records_func(Path(backtest_parquet), price_col=price_col, initial_capital=initial_capital, time_format=time_format, points=points)
            source = "parquet"
        ...
        if default_demo.exists():
            try:
                ...
                raise HTTPException(status_code=500, detail=f"failed reading default demo json: {e}")
        else:
            ...
            if not candidate.exists():
                raise HTTPException(status_code=404, detail="No chart data found. Provide a backtest_parquet or place precomputed CSV at results/sentiment_2025-07-01_to_2025-08-01.csv")
            ...
            if not price_parquet.exists():
                LOG_ERR(f"Price parquet not found at {price_parquet}; cannot run backtest automatically")
            ...
            if prepare_dataset_func is None or run_backtest_func is None or build_records_func is None:
                LOG_ERR("One of prepare_dataset/run_backtest/build_records functions is missing; cannot complete pipeline automatically")
                return JSONResponse(status_code=202, content={
                    "status": "generated",
            ...
            try:
                start_ts = parse_hourly_datetime(startDateTime) if startDateTime and parse_hourly_datetime else None
                end_ts = parse_hourly_datetime(endDateTime) if endDateTime and parse_hourly_datetime else None
                prepared_parquet = Path("data/prepared_runtime.parquet")
                backtest_parquet = Path("results/backtest_runtime.parquet")

                LOG(f"[Debug] Preparing dataset from price={price_parquet} sentiment={sentiment_csv_path} -> {prepared_parquet}")
                prepare_dataset_func(price_parquet=str(price_parquet), sentiment_csv=str(sentiment_csv_path), out_path=str(prepared_parquet), decay_span=12, start_time=start_ts, end_time=end_ts)

                LOG(f"[Debug] Running backtest on prepared dataset -> {backtest_parquet}")
                run_backtest_func(input_parquet=Path(prepared_parquet), price_col=price_col, sentiment_col="sentiment_score", output_path=Path(backtest_parquet), params=None)

                LOG(f"[Debug] Building records from backtest parquet {backtest_parquet}")
                records = build_records_func(Path(backtest_parquet), price_col=price_col, initial_capital=initial_capital, time_format=time_format, points=points)
            except Exception as e:
                LOG_ERR(f"Automatic pipeline failed: {e}")
                raise HTTPException(status_code=500, detail=f"Automatic pipeline failed: {e}")
    ...
    if records and (startDateTime or endDateTime):
        try:
            s_req = datetime.fromisoformat(startDateTime.replace("Z", "+00:00")).astimezone(timezone.utc) if startDateTime else None
        except Exception:
            s_req = None
        try:
            e_req = datetime.fromisoformat(endDateTime.replace("Z", "+00:00")).astimezone(timezone.utc) if endDateTime else None
        except Exception:
            e_req = None
        ...
        if isinstance(records, list):
            ...
            try:
                ...
                if isinstance(records, _pd.DataFrame):
                    df_rec = records.copy()
                    df_rec['time'] = _pd.to_datetime(df_rec['time'], utc=True)
